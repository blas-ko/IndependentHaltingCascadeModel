...
class IndependentHaltingCascadeModel():
    ...
    def __init__(self, edgelist, nodelist=None, activation_probs=None, application_probs=None, hiring_probs=None) -> None:
        ...
        Args:
            ...
            activation_probs (_type_, optional): _description_. Defaults to None.
            application_probs (_type_, optional): _description_. Defaults to None.
            hiring_probs (_type_, optional): _description_. Defaults to None.
        """                
        ...
        if activation_probs is not None:
            self.edgelist['activation_probs'] = activation_probs
    ...
    def simulate(
        ...
        """Simulates the the independent cascade model using the `initial_spreaders` as seed nodes.

        Args:
            edgelist (pandas.dataframe): network represented with an edgelist with columns ['source','target',activation_probability_col]
            initial_spreaders (list): list of seed node ids. The ids should be in the 'source' column of `edgelist`.
            max_diffusion_time (int, optional): Maximum cascade depth of the model after which the model stops. Defaults to 10.
            activation_prob_col (str, optional): name of the column in edgelist containing the edge activation probabilities. Defaults to 'activation_probability'.

        Returns:
            diffusion_steps (list): the i'th element of this list is a list with the nodes activated at the i'th step of the model.
            activated_nodes (set): set of all node ids activated in the model simulation.
        """    
    ...
    def _step(self, spreaders, sources=None, edgelist=None):
        ...
        activations_mask = self._get_activations( edgelist=spreaders_neighs )
    ...
    def _get_activations(self, edgelist, random_state=None):
        ...
        activation_probabilities = edgelist.set_index('target')['activation_probs']
        # Determine which targets were activated and which ones were not
        successful_activations = activation_probabilities > activation_thresholds
